...
cat tmp_substring | cut -d ':' -f2 | sort | tee tmp_passwords &>/dev/null && ./scripts/extensions/common-substr-mac -n -f tmp_passwords > tmp_allsubstrings && rm tmp_passwords tmp_substring
